#!/bin/bash
:"
	Script de download dos arquivos de Campanhas nas lojas Prisma.
...
DIR_REMOTO="/lasa/usr/PRODUCAO/COMNC/STATX/CAMPANHA/LJ${LOJA}"
...
mkdir -p ${DIR_LOCAL}/CAMPANHA/{BACKUP,TRAB}
...
for TRY in $(seq 5); do
	...
	sleep ${TEMPO}
	${RSYNC} --rsh="${SSH}" ${SERVER}:${DIR_REMOTO}/CAMPANHA.????.??????????????.tbz2 ${DIR_LOCAL}/CAMPANHA/TRAB >>${LOG} 2>&1
	EXIT=$?
	if [ ${EXIT} -eq 0 ]; then
		SUCESSO="1"
...
if [ ${SUCESSO} -eq 1 ]; then
	cd ${DIR_LOCAL}/CAMPANHA/TRAB
	echo -e "$(data_hora) ${HOST} - Descompactando arquivos." >>${LOG} 2>&1
	tar xjf CAMPANHA.*.*.tbz2
	EXIT=$?
	if [ ${EXIT} -eq 0 ]; then
		echo -e "$(data_hora) ${HOST} - Testando arquivos." >>${LOG} 2>&1
		MD5NOVO=$(md5sum brindes.*.txt levpag.*.txt 2>/dev/null |awk '{print $1}' |md5sum 2>/dev/null |awk '{print $1}')
		MD5ATUAL=$(md5sum ${DIR_LOCAL}/brindes.txt ${DIR_LOCAL}/levpag.txt 2>/dev/null |awk '{print $1}' |md5sum 2>/dev/null |awk '{print $1}')
		if [ ${MD5NOVO} != ${MD5ATUAL} ]; then
			echo -e "$(data_hora) ${HOST} - Atualizando arquivos:" >>${LOG} 2>&1
			rm -fv ${DIR_LOCAL}/brindes.txt ${DIR_LOCAL}/levpag.txt >>${LOG} 2>&1
			mv -fv brindes.*.txt ${DIR_LOCAL}/brindes.txt >>${LOG} 2>&1
			mv -fv levpag.*.txt ${DIR_LOCAL}/levpag.txt >>${LOG} 2>&1
			...
			rm -fv brindes.*.txt levpag.*.txt >>${LOG} 2>&1
		fi
	else
	...
	mv -fv ${DIR_LOCAL}/CAMPANHA/TRAB/CAMPANHA.*.*.tbz2 ${DIR_LOCAL}/CAMPANHA/BACKUP >>${LOG} 2>&1
fi
