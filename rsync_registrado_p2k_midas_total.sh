...
#   Versao 1.3 (23/03/2020)
#       - Todas lojas Midas
#   Versao 1.4 (25/03/2020)
#       - Verificacao de execucao do processo Prisma
#   Versao 1.5 (25/03/2020)
#       - Criada area intermediaria para transmissao para o Midas
#
####################################################################################################
...
PIDPRISMA="/DSOP/DLOG/rsync_registrado_prisma.pid" ## Arquivo de PID Prisma
...
function verifica_pid_prisma {
	if [ -s ${PIDPRISMA} ]; then
		LASTPIDPRISMA=$(cat ${PIDPRISMA})
		if [ -d /proc/${LASTPIDPRISMA} ]; then
			echo "`date \"+%F %T\"` - Processo Prisma em execucao." >>${LOG} 2>&1
			exit 0
		fi
	fi
}
...
for DIRLJ in `ls -d ${DIRFS}/LJ????`; do
	...
	if [ "`find ${DIRLJ}/Exportacao/* -prune -type f \( -name lfin* -o -name lout* -o -name lrin* -o -name LVDEP* -o -name tes* \) 2>/dev/null |wc -l`" -gt "0" ]; then
		...
		#	find ${DIRLJ}/Exportacao/* -prune -type f \( -name lfin* -o -name lout* -o -name lrin* -o -name LVDEP* -o -name tes* \) -exec rsync -cgopv --remove-source-files --timeout=30 {} $exec rsync -cgopv --remove-source-files --timeout=30 {} ${DIRTES} \; >>${L{DIRTES} \; >>${LOG} 2>&1
		#else
			find ${DIRLJ}/Exportacao/ -type f \( -name lfin* -o -name lout* -o -name lrin* -o -name LVDEP* -o -name tes* \) -exec rsync -cgopq --timeout=30 {} ${DIRBKP}/LJ${LJ} \; >>${LOG} 2>&1
...
find ${DIRFS}/LJ????/Exportacao/ -type f \( -name lfin* -o -name lout* -o -name lrin* -o -name LVDEP* -o -name tes* \) -exec mv -f {} /RSYNC/MIDAS/REGISTRADO \; >>${LOG} 2>&1 && \
rsync -v --timeout=30 /RSYNC/MIDAS/REGISTRADO/* ${DIRTES} >>${LOG} 2>&1 && \
rsync -gopv --remove-sent-files --timeout=30 /RSYNC/MIDAS/REGISTRADO/* ${USRMIDAS}@${SRVMIDAS}:${DIRMIDAS} >>${MLOG} 2>&1
